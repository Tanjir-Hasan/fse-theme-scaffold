...
add_filter( 'admin_bar_menu', {{CONST_PREFIX}}_NS . 'add_admin_menu', 90, 1 );
...
function add_admin_menu( \WP_Admin_Bar $admin_bar ):void {
	...
	$admin_bar->add_menu(
		array(
			'id'     => '{{SLUG}}-edit-template',
			'parent' => null,
			'group'  => null,
			'title'  => '<span class="ab-icon dashicons-edit" style="top:2px"></span>' . __( 'Edit template', '{{SLUG}}' ),
			'href'   => admin_url( 'site-editor.php?postType=wp_template&postId=' . $id . '' ),
			'meta'   => array(
				'title' => __( 'Edit template', '{{SLUG}}' ), // This title will show on hover.
			),
		) 
	);
}
